...
* 適応型遺伝的アルゴリズムによる適切な音声の学習
* 音源の特性に合わせて起床時間を最小化するようにパラメータを自動調整して学習を行っています。
適応型遺伝的アルゴリズムとは生物の進化のように、良い設定（遺伝子）を残しつつ、少しずつ変化（変異）させて最適解を探す手法であり例えば起床時間が遅い時には変化を大きくするなどの処理をしてユーザーの慣れを減らします。遺伝的アルゴリズムに用いられる突然変異という仕組みを用いて大きく変化したピッチや速度での音を用いることで局所解にハマらず真の最適解をみつけられるようにしています。またパラメータの正規化を行うことでスピードとピッチどちらの探索速度を極端に起こさないようにしている。
