...
export default function HomeScreen() {
  ...
  const { isMonitoring, volume, magnitude, isAlarmActive } = useAlarmLogic(testTarget);
  ...
  return (
    <View style={[styles.container, isAlarmActive && styles.containerAlert]}>
      ...
      <View style={styles.card}>
        ...
        <Text style={[styles.value, { color: isMonitoring ? '#4CAF50' : '#F44336' }]}>
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.value}>{(volume * 100).toFixed(0)} %</Text>
      </View>
...
const styles = StyleSheet.create({
  container: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#f0f0f7' },
  // アラーム作動時に背景色を変えるとかっこいい
  containerAlert: { backgroundColor: '#FFEBEE' }, 
  title: { fontSize: 28, fontWeight: 'bold', marginBottom: 20 },
  card: { backgroundColor: '#fff', padding: 20, borderRadius: 15, width: '80%', marginBottom: 15, elevation: 3 },
